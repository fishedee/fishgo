...
func TestQueryJoin(t *testing.T) {
	...
	AssertEqual(t, QueryLeftJoin(admin, user, "AdminId = UserId", func(admin Admin, user User) AdminUser {
		return AdminUser{
...
func BenchmarkQueryJoinMacro(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i != b.N; i++ {
		QueryLeftJoin(admin, user, "AdminId = UserId", func(admin Admin, user User) AdminUser {
			return AdminUser{
				AdminId:    admin.AdminId,
				Level:      admin.Level,
				Name:       user.Name,
...
func BenchmarkQueryJoinReflect(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i != b.N; i++ {
		QueryLeftJoin(admin, user, "AdminId = Age", func(admin Admin, user User) AdminUser {
			return AdminUser{
				AdminId:    admin.AdminId,
				Level:      admin.Level,
				Name:       user.Name,
