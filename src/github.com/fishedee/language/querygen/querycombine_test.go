...
func TestQueryCombine(t *testing.T) {
	...
	AssertEqual(t, QueryCombine(admin, user, func(admin Admin, user User) AdminUser {
		return AdminUser{
...
func BenchmarkQueryCombineMacro(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i != b.N; i++ {
		QueryCombine(admin, user, func(admin Admin, user User) AdminUser {
			return AdminUser{
				AdminId:    admin.AdminId,
				Level:      admin.Level,
				Name:       user.Name,
...
func BenchmarkQueryCombineReflect(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i != b.N; i++ {
		QueryCombine(user, admin, func(user User, admin Admin) AdminUser {
			return AdminUser{
				AdminId:    admin.AdminId,
				Level:      admin.Level,
				Name:       user.Name,
